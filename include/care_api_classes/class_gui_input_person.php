...
class GuiInputPerson {
	 ...
	 */
	function display() {
		global $db, $sid, $base_url, $lang, $root_path, $pid, $insurance_show, $user_id, $mode, $dbtype, $no_tribe, $no_region, $update, $photo_filename; //, $HTTP_POST_FILES $HTTP_POST_VARS, $HTTP_SESSION_VARS;
...
?>
		...
		function test(){
			document.aufnahmeform.action="<?php echo $_SERVER['PHP_SELF'];?>";
			...
			<?php
				...
				if (!$update OR $region == '') {
					echo '} else if (d.region.value=="-1"){
							...
							d.region.focus();return false;
						} else if (d.district.value=="-1"){
							alert("Please select district");
							d.district.focus();
							return false;
						} else if (d.ward.value=="-1")  {
...
?>js/dtpick_care2x.js"></script>
		...
		<script src="<?php
		echo $root_path;
?>modules/dental/dental_filling_.js" language="javascript"></script>
		<script src="<?php
		echo $root_path;
?>modules/dental/JQ.js" language="javascript"></script>
...
?>&nbsp;
			...
			<td  rowspan=6 class="photo_id" >
				<FONT SIZE=-1  FACE="Arial">
				<a href="#"  onClick="showpic(document.aufnahmeform.photo_filename)"><img <?php //echo $img_source;
...
?>:
			...
			<td class="reg_input">
				<FONT SIZE=-1  FACE="Arial" color="#800000">
				<?php
		echo formatDate2Local($date_reg, $date_format, FALSE, FALSE, '');
?>
...
?>
			...
			<td class="reg_input">
				<input type="text" name="selian_pid" size=14 maxlength=11 value="<?php //echo $selian_pid
?>" onFocus="this.select();">
			</td>
			</tr>
			-->
...
?>:
			...
			<td class="reg_input">
				<input type="text" name="allergy" size=14 maxlength=25 value="<?php
		echo $allergy;
?>" onFocus="this.select();">
			</td>
			</tr>
...
?>:
			...
			<td class="reg_input">
				<input type="text" name="employer" size=14 maxlength=25 value="<?php
		echo $employer;
?>" onFocus="this.select();">
			</td>
...
?></td>
				<td  class="reg_input" colspan=1 id="dstr">
							<select name="district" size="1" id="district" onChange="getCounty(this.value,'<?php
		echo $base_url;
?>')">
							<?php
...
?></font>:
			...
			<tr>
			<td class="reg_item">
				<FONT SIZE=-1  FACE="Arial" ><FONT  SIZE=2  FACE="Arial"><?php
		echo $LDCounty;
?>
			</td>
			<td class="reg_input">
...
?>
			...
			<td class="reg_input">
				<input type="text" name="email" size=60 maxlength=60 value="<?php
		echo $title;
?>" onFocus="this.select();">
			</td>
			</tr>
...
?>
			...
			<td class="reg_input">
				<input type="text" name="NOKName" size=25 maxlength=32 value="<?php
		echo $NOKName;
?>" onFocus="this.select();">
			</td>
			</tr>
...
?>
			...
			<td class="reg_input">
				<input type="text" name="NOKRelation" size=17 maxlength=25 value="<?php
		echo $NOKRelation;
?>" onFocus="this.select();">
			</td>
			</tr>
...
?>
			...
			<td class="reg_input">
				<input type="text" name="NOKTel" size=15 maxlength=25 value="<?php
		echo $NOKTel;
?>" onFocus="this.select();">
			</td>
			</tr>
