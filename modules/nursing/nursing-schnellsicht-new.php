...
//$db->debug=1;
 ...
 $dbtable='care_ward';
  ...
  if($is_today)
   $sqlSnippet.=" AND '$s_date'>=l.date_from AND l.date_to = '$dbf_nodate'";
  else
   $sqlSnippet.=" AND '$s_date'>= l.date_from AND '$s_date' <='".date('Y-m-d')."' AND ('$s_date'<=l.date_to OR l.date_to = '$dbf_nodate')";

  $sqlSnippet.=" LEFT JOIN care_encounter ce ON l.encounter_nr = ce.encounter_nr ";
