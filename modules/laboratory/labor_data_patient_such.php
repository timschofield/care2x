...
*/
	...
	while($zeile=$encounter->FetchRow()){
		...
		//if($zeile['encounter_nr']!=$prev_nr){
			...
			$is_missing_res = $lab_obj->IsMissingLabResult($zeile['batch_nr'],$zeile['encounter_nr']);
			if ($is_missing_res==1)
		     echo ' <img '.createComIcon($root_path,'check2.gif','0','',TRUE).'></img>'; # all results are in the list
			elseif ($is_missing_res==-1)
			...
			//if there are any results for this request, present a link to it
			if ($is_missing_res!=-2) {
				if($editmode){
