...
<table cellspacing="0"  class="titlebar" border=0>
 <tr valign=top  class="titlebar" >
  <td bgcolor="#99ccff">&nbsp;&nbsp;<font color="#330066">Education on basics, prevention, disclosure </font></td>
...
<form method="post" name="form_education" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
  ...
  onFocus="this.select()" onKeyUp="setDate(this,'<?php echo $date_format; ?>','<?php echo $lang; ?>')" value="<?php echo $values['comment_date'] ?>" size="10" maxlength="10" />
        <a href="javascript:show_calendar('form_education.comment_date','<?php echo $date_format; ?>')"> <img src="<?php echo $root_path; ?>gui/img/common/default/show-calendar.gif" border=0 align="absmiddle" width="26" height="22"></a>(dd/mm/yyyy) </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
  ...
  <tr bgcolor="#EAF5FF">
    <td width="35" >
      <strong><?php echo $messages['comment_id']."\n" ?></strong>
      <input name="comment_id" type="radio" value="1" <?php echo $values['comment_id'] == 1 ? 'checked="checked" ' : '' ?> /></td>
    <td width="721" ><strong>Basic HIV education, <br />
    transmission</strong></td>
	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[1][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="2" <?php echo $values['comment_id'] == 2 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Prevention: abstinence, safer sex, condoms </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[2][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#E8F2FF">
    <td><input name="comment_id" type="radio" value="3" <?php echo $values['comment_id'] == 3 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Prevention: household precautions, what is safe </strong></td>
   	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[3][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="4" <?php echo $values['comment_id'] == 4 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Post-test counselling: implications of results </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[4][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#E8F2FF">
    <td><input name="comment_id" type="radio" value="5" <?php echo $values['comment_id'] == 5 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Positive living </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[5][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="6" <?php echo $values['comment_id'] == 6 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Testing partners </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[6][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#E8F2FF">
    <td><input name="comment_id" type="radio" value="7" <?php echo $values['comment_id'] == 7 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Disclosure</strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[7][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="8" <?php echo $values['comment_id'] == 8 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>To whom disclosed (list) </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[8][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#E8F2FF">
    <td><input name="comment_id" type="radio" value="9" <?php echo $values['comment_id'] == 9 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Familiy/living sitution </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[9][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="10" <?php echo $values['comment_id'] == 10 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Shared confidentially </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[10][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#E8F2FF">
    <td><input name="comment_id" type="radio" value="11" <?php echo $values['comment_id'] == 11 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Reproductive choices, prevention MTCT </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[11][$i]."&nbsp;</td>";
	}
    ?>
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="12" <?php echo $values['comment_id'] == 12 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Child's blood test </strong></td>
    <?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[12][$i]."&nbsp;</td>";
	}
    ?>  
  </tr>
</table>
