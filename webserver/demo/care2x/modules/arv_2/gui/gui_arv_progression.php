...
<table cellspacing="0"  class="titlebar" border=0>
 <tr valign=top  class="titlebar" >
  <td bgcolor="#99ccff">&nbsp;&nbsp;<font color="#330066">Progesssion, Rx </font></td>
...
<form method="post" name="form_progression" action="<?php echo $_SERVER['PHP_SELF']; ?>">
  <table width="460" border="0" bgcolor="#F0F8FF" class="mainTable">
   ...
   onFocus="this.select()" onKeyUp="setDate(this,'<?php echo $date_format; ?>','<?php echo $lang; ?>')" value="<?php echo $values['comment_date']  ?>" size="10" maxlength="10" />
        <a href="javascript:show_calendar('form_progression.comment_date','<?php echo $date_format; ?>')"> <img src="<?php echo $root_path; ?>gui/img/common/default/show-calendar.gif" border=0 align="absmiddle" width="26" height="22"></a>(dd/mm/yyyy) </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
  ...
  <tr bgcolor="#EAF5FF">
    <td width="35">
      ...
      <input name="comment_id" type="radio" value="13" <?php echo $values['comment_id'] == 13 ? 'checked="checked" ' : '' ?>/>    </td>
    <td width="721"><strong>Progession of disease </strong></td>
	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[13][$i]."&nbsp;</td>";
	}
    ?>  
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="14" <?php echo $values['comment_id'] == 14 ? 'checked="checked" ' : '' ?>/></td>
    <td><strong>Available treatment/prophylaxis </strong></td>
	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[14][$i]."&nbsp;</td>";
	}
    ?>  
  </tr>
  <tr bgcolor="#EAF5FF">
    <td><input name="comment_id" type="radio" value="15" <?php echo $values['comment_id'] == 15 ? 'checked="checked" ' : '' ?> /></td>
    <td><strong>Follow-up appointments, care and treatment team </strong></td>
	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[15][$i]."&nbsp;</td>";
	}
    ?>  
  </tr>
  <tr bgcolor="#F0F8FF">
    <td><input name="comment_id" type="radio" value="16" <?php echo $values['comment_id'] == 16 ? 'checked="checked" ' : '' ?> /></td>
    <td><strong>CTX, INH prophylaxis </strong></td>
	<?php
	for($i=0; $i<$max_comments; $i++) {
		echo "<td>".$comments[16][$i]."&nbsp;</td>";
	}
    ?>  
  </tr>
</table>
