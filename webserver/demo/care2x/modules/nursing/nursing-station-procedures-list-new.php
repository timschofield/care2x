...
<?php
/* NOTE: Date not relevant for procedures: always show them all for current encounters
			...
			echo '<tr class="titlebar" bgcolor="#99ccff"><td><font color="#330066">Procedures for Current Admissions</font></td></tr>';
			echo '<tr class="titlebar" bgcolor="#99ccff"><td>&nbsp;</td></tr>';
...
<?php
			...
			foreach ($coreObjOuter->result as $rowEncounter){
				...
				//NOTE: Date is NOT part of search criteria for procedures
				...
				$sqlInner = "SELECT * FROM care_encounter_prescription INNER JOIN care_tz_drugsandservices ON care_tz_drugsandservices.item_id=care_encounter_prescription.article_item_number" .
			                    " WHERE encounter_nr = $encounterNr and purchasing_class in ('minor_proc_op','obgyne_op','ortho_op','surgical_op')".
				...
				foreach ($coreObjInner->result as $rowPrescr){
					...
					if ($prescr!='')
						$prescr .= '<tr><td>&nbsp;</td><td>&nbsp;</td>';

					$article = $rowPrescr['article'];
					$modify_time = formatDate2Local($rowPrescr['modify_time'],$date_format,true);
					$prescriber = $rowPrescr['prescriber'];
					$purchasing_class = $rowPrescr['purchasing_class'];
					$notes = $rowPrescr['notes'];
					$prescr .= "<td>".$article."</td>
								<td>".$modify_time."</td>
								<td>".$prescriber."</td>
								<td>".$purchasing_class."</td>
								</tr>";				
				}
				if ($prescr == '')
