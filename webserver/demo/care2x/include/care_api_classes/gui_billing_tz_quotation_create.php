		...
		else
		{
			...
			$result = $bill_obj->GetNewQuotation_Laboratory($_REQUEST['encounter_nr'],'');
		 	...
		 	while($row=$result->FetchRow())
		 	{
				...
				echo "	if (document.forms[0].elements['modelab_".$row['sub_id']."'][i].checked==true){ ";
				echo "if(document.forms[0].elements['insurance_' + id]) {";
		 		echo "sum = document.forms[0].elements['showprice_' + id].value * document.forms[0].elements['dosage_' + id].value; \n";
				echo "sum_total = sum - document.forms[0].elements['insurance_' + id].value; \n";
				echo "document.getElementById('div_' + id).innerHTML='<table width=\"100%\" border=\"1\"><tr><td>' + document.forms[0].elements['unit_price_' + id].value + ' x ' + document.forms[0].elements['dosage_' + id].value + ' = </td><td align=\"right\">' + sum + ' TSH</td></tr><tr><td>Insurance:</td><td align=\"right\">- ' + document.forms[0].elements['insurance_' + id].value + ' TSH</td></tr><tr><td><b>Sum:</b></td><td align=\"right\"><b>' + sum_total + ' TSH</b></td></tr></table><input type=\"hidden\" name=\"pressum_' + id + '\" value=\"'+ sum_total + '\">'; \n";
				echo '  } else {';
				echo "sum=document.forms[0].elements['unit_price_' + id].value * document.forms[0].elements['dosage_' + id].value; \n"; 
 				echo "sum_total = sum; \n";
	 			echo "document.getElementById('div_' + id).innerHTML='<table width=\"100%\" border=\"1\"><tr><td>' + document.forms[0].elements['unit_price_' +id].value + ' TSH </td><td align=\"right\"></td><input type=\"hidden\" name=\"pressum_' + id + '\" value=\"'+ sum_total + '\">'; \n";
		 		echo "}";
		 		echo "}";
		 		echo "}";
		 	}
