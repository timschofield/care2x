 ...
 *  2011-04-05	MarosC	Numerous fixes:
 *  						- added insertDataFromInternalArray() to update modify date on results save
 *  						- resorted results desc by date
 *  						- re-wrote IsMissingLabResult to use encounter_nr and left join vs. itterate in code
 */
...
($this->dead_stat)";
	...
	function getAllResults($enc_nr = '') {
		...
		if (! $this->internResolveEncounterNr ( $enc_nr ))
			return FALSE;
		$this->sql = "SELECT * FROM $this->tb_find_chemlab INNER JOIN $this->tb_find_chemlab_sub ";
		$this->sql .= "ON ($this->tb_find_chemlab.job_id = $this->tb_find_chemlab_sub.job_id AND $this->tb_find_chemlab.encounter_nr = $this->tb_find_chemlab_sub.encounter_nr) ";
		$this->sql .= "WHERE $this->tb_find_chemlab.encounter_nr='$this->enc_nr' AND $this->tb_find_chemlab.status NOT IN ($this->dead_stat) ORDER BY $this->tb_find_chemlab_sub.test_date DESC";
		if ($this->result = $db->Execute ( $this->sql )) {
			if ($this->rec_count = $this->result->RecordCount ()) {
				return $this->result;
			} else {
	...
	function GetTestsToDo($id = '', $encounter_nr= '') {
		global $db;
		if (empty ( $id ))
			$cond = '';
		else
			$cond = "batch_nr=".$id." AND encounter_nr=".$encounter_nr;
		$sub = "_sub";
...
unsorted result.
	 ...
	 * 
	 * @param $batch_nr (or job_id in results table)
	 * @param $encounter_nr encounter/admission number
	 * return:
...
the database
	 ...
	 */
	function IsMissingLabResult($batch_nr,$encounter_nr) {
		global $db;
		$debug = FALSE;
		$param_array = array ();
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		$this->sql = "SELECT req.paramater_name, res.parameter_value as 'result_value'
					FROM care_test_request_chemlabor_sub req LEFT JOIN care_test_findings_chemlabor_sub res
						on req.batch_nr = res.job_id and req.encounter_nr = res.encounter_nr and req.paramater_name = res.paramater_name
					WHERE req.batch_nr = ".$batch_nr." AND req.encounter_nr = ".$encounter_nr;
		
		$requests_with_results = 0;
		$total_requests = 0;
		if ($this->result = $db->Execute ( $this->sql )) {
			$total_requests = $this->result->RecordCount();
			while ($row = $this->result->FetchRow()) {
				if (!isset($row['result_value']) || empty ($row['result_value']) || $row['result_value']==DBF_NODATE) {
					//no result
				} else {
		...
		if ( ($requests_with_results == $total_requests) && $total_requests > 0) {
			if ($debug)
				echo "all results are given! return code true"; 
			return 1;
		} else if ( ($requests_with_results < $total_requests) && $requests_with_results != 0) {
			if ($debug)
...
($this->dead_stat) ORDER BY modify_time DESC";
	...
	/**
	* Helper function to Core::insertDataFromInternalArray() use to update modify_time on 
	* $this->fld_find_chemlab_sub when sub_table is set to true.
	*
	* @access public
	* @return boolean
	*/
	function insertDataFromInternalArray() {
		...
		$debug = false;
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		if ($this->coretable == $this->tb_find_chemlab_sub) {
			//we are updating tb_find_chemlab_sub table
			$tempSql = "UPDATE $this->tb_find_chemlab SET
	        modify_time= '" . date('YmdHis') . "'
	        WHERE job_id=" . $this->data_array['job_id'] .
			" AND encounter_nr=" . $this->data_array['encounter_nr'];

			if ($debug) echo $tempSql;
			
			if (!$this->Transact($tempSql)) {
				//save failed, abort saving
				if ($debug) echo 'failed save!';
				return FALSE;				
			}
		}

		//continue with parent save
		return parent::insertDataFromInternalArray();						
	}
