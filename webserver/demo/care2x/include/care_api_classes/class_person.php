...
class Person extends Core {
	...
	* @var array
	*/
	var $basic_list='pid,title,name_first,name_last,name_2,name_3,name_middle,name_maiden,name_others,date_birth,employer,parish,subcounty,NOKName,NOKTel,NOKRelation,blood_group,sex,addr_str,addr_str_nr,addr_zip,addr_citytown_nr,citizenship,photo_filename,email,sss_nr,nat_id_nr,region,district,ward';
	/**
	...
	*/
	function getAllInfoObject($pid='') {
	    ...
	    $this->sql="SELECT p.*, citizenship AS addr_citytown_name,ethnic.name AS ethnic_orig_txt, tribe.tribe_name , religion.name ,region.region_name  ,district.district_name ,ward.ward_name
					...
					LEFT JOIN care_ug_tribes AS tribe ON p.name_maiden=tribe.tribe_id
