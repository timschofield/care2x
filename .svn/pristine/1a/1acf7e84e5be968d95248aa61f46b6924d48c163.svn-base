	...
	{//echo print_r($_POST);
		...
		elseif(strstr($x,"modelab_"))
		{
			...
			if ($dosageOld != $dosageNew)
				...
				if ($_POST['insurance']!=-1)
					$insurance_obj->allocateLaboratoryItemsToinsurance($new_bill_number, $labtest_nr, $_POST['showprice_'.$labtest_nr],
$_POST['insurance']);
			}
		...
		elseif(strstr($x,"moderad_"))
		{
			...
			if ($dosageOld != $dosageNew)
				...
				if ($_POST['insurance']!=-1)
					$insurance_obj->allocateRadiologyItemsToinsurance($new_bill_number, $radtest_nr, $_POST['showprice_'.$radtest_nr],
$_POST['insurance']);
			}
	...
	if($billcounter){
		...
		$bill_obj->StoreAdvancePaymentToBill($pid,$new_bill_number,0,$balance,$notes);

		$bill_obj->TransferBillItemsWithAdvance($new_bill_number);
