...
$sql_tmp_ward = "CREATE TEMPORARY TABLE tmp_ward TYPE=HEAP (SELECT nr,ward_id,name,is_temp_closed,status FROM care_ward)"; 
$db->Execute($sql_tmp_ward);

$sql_e ="DROP TEMPORARY TABLE IF EXISTS `tmp_table`";
$db->Execute($sql_e);
		
$sql_tmp_enc = "CREATE TEMPORARY TABLE tmp_table TYPE=HEAP (SELECT encounter_nr,encounter_date,current_ward_nr FROM care_encounter 
WHERE UNIX_TIMESTAMP(encounter_date) >= '$start' AND UNIX_TIMESTAMP(encounter_date) <= '$end')";  
$db->Execute($sql_tmp_enc);



require_once('gui/gui_IPD_admissions.php');
?>