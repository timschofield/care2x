...
/**
* This script is included by the barcode-etik.php script for arabic languages
*/
...
#        fathalla_w@hotmail.com       
 	...
 	include_once($root_path.'include/inc_ttf_ar2uni.php');
		...
		$locstr.=' '.$admit_type.' ';
		$locstr.=ar2uni($LDInsShortID[$result['insurance_class_nr']]);
	...
	if($ttf_ok){
    	...
    	ImageTTFText($label,11,0,$lmargin,$tmargin+108,$eblack,$arial,ucfirst($result['addr_zip']).' '.ar2uni($result['name']));
		# Sex
	...
	if($ttf_ok){
    	...
    	ImageTTFText($label2,10,0,10,109,$black,$arial,ar2uni($ins_obj->getFirmName($result['insurance_firm_id'])));
