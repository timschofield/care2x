...
function item_add() {
  ...
  var destination_obj       = document.forms[0].elements['selected_order_list[]'];
...
function submit_form(reference, t, session, todo) {
  // (c) by Merotech (AI) 2006
  
  var destination_obj = document.forms[0].elements['selected_item_list[]'];
  var order_obj = document.forms[0].elements['selected_order_list[]'];
  var parameters="reference="+reference.name + "&supplier="+document.forms[0].elements['supplier'].value;
	switch(todo)
	{
		case "search":
			try
		...
		case "done":
			try
			{
				var druglist_obj = document.forms[0].elements['druglistitems[]'];
				
				if (druglist_obj.length > 0)
				{
						    for (var i=0 ; i<druglist_obj.length; i++ )
					    	if(druglist_obj.options[i].value!=-1)
					      	parameters+="&druglist_no["+i+"]="+druglist_obj.options[i].value;
				}
				else
				{
	...
	try
	{
	  ...
	  if (order_obj.length > 0)
	    for (var i=0 ; i<order_obj.length; i++ )
	    	if(order_obj.options[i].value!=-1)
	      	parameters+="&order_no["+i+"]="+order_obj.options[i].value;
  }
  catch(E){}
